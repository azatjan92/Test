...
@section('main_content')
    ...
    <div class="container" style="max-width: 1200px; margin: 50px auto;">
        @php
            $user = auth()->user();
            $feedbacks = \App\Models\Feedback::paginate(10);
        @endphp
        ...
        @else
            <h2 style="margin-top: 40px">@lang('messages.feedback')</h2>
            <form id="feedbackForm" method="POST" action="{{ route('feedback.store') }}">
                @csrf
                <div class="form-group" style="margin-top: 20px">
                    <label for="name">@lang('messages.name')</label>
                    <input type="text" id="name" name="name" class="form-control" value="{{ auth()->check() ? auth()->user()->name : '' }}">
                </div>
                <div class="form-group" style="margin-top: 20px">
                    <label for="email">Email</label>
                    <input type="email" id="email" name="email" class="form-control" value="{{ auth()->check() ? auth()->user()->email : '' }}">
                </div>
                <div class="form-group" style="margin-top: 20px">
                    <label for="message">@lang('messages.sms')</label>
                    <textarea id="message" name="message" class="form-control" required></textarea>
                </div>
                <button style="margin-top: 20px" type="submit" class="btn btn-primary">@lang('messages.send')</button>
            </form>
        @endif
    </div>
@endsection
